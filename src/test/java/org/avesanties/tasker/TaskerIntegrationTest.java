...
class TaskerIntegrationTest extends TaskerApplicationTestConfiguration {
	...
	void whenGetByPeriod_thenOK() throws JsonMappingException, JsonProcessingException {
		...
		URI uri = uriBuilderFactory.builder()
				...
				.queryParam("dateEnd", LocalDate.now().plusDays(100L))
				.build();
		...
		List<Task> tasks = objectMapper.readValue(response.getBody(), new TypeReference<List<Task>>(){ });
	...
	void whenGetByPeriodAndState_thenBadRequest(){
		URI uri = uriBuilderFactory.builder()
				...
				.queryParam("dateEnd", LocalDate.now().minusDays(100L))
				.build();
